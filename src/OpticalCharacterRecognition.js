    ...
    static setupSelectFromAvailableModels(selectElement, getFileList)
    {
        ...
        getFileList(OpticalCharacterRecognition.TESSERACT_LANG_PATH).forEach(file => {
            if (file.endsWith(OpticalCharacterRecognition.TESSERACT_CORE_EXTENSION)) {
                ...
                option.value = lang;
                option.textContent = OpticalCharacterRecognition.TESSERACT_LANG_EXCEPTION_MAP[lang];
                if (!option.textContent)
                {
                    option.textContent = lang.slice(0, 2);
                }
                selectElement.appendChild(option);
                if (langCode == option.textContent)
                {
...
OpticalCharacterRecognition.TESSERACT_LANG_EXCEPTION_MAP ={"spa":"es"};
